...
\begin{figure}
    ...
    \end{minipage}
    \caption{The mesh scanned using Kinfu and rigged using Pinocchio. The skeleton is shown in olive. \fixme{TODO: change to an animated version}}
...
\subsection{Parametric human body model}
...
\fixme{TODO: write this so that it makes sense and reflects our practical achievements}
...
A confidence measure could be used, so that points far from the surface have a smaller effect on the final color. Practically a Gaussian function of the form
\begin{align*}
    f \colon \mathbb{R}_{\ge 0} & \to \mathbb{R}_+ \\
                              x & \mapsto a e^{-b x^2} \mid a, b \in \mathbb{R}_+
\end{align*}
